<template>
  <div>
    <template>
      <el-card class="box-card one-card-one" shadow="never" >
    <div class="top-text-1">
      <span class="top-text-1-1">工单统计</span>
      <span class="top-text-1-2">{{timer.start+'~'+timer.end}}</span>
    </div>
    <div class="buttom-text-1">
      <div class="buttom-text-1-1">
        <span class="buttom-text-1-1-1">{{CardOne.total}}</span>
        <span class="buttom-text-1-1-2">{{CardOne.completedTotal}}</span>
        <span class="buttom-text-1-1-3">{{CardOne.progressTotal}}</span>
        <span class="buttom-text-1-1-4">{{CardOne.cancelTotal}}</span>
      </div>
      <div class="buttom-text-1-2">
        <span>工单总数（个）</span>
        <span>完成工单（个）</span>
        <span>进行工单（个）</span>
        <span>取消工单（个）</span>
      </div>
    </div>
     </el-card>
    </template>
  </div>
</template>

<script>
export default {
  props: ['CardOne'],
  created () {

  },
  computed: {
    timer () {
      const timer = {
        start: this.$store.getters.time.start.replace(/-/g, '.'),
        end: this.$store.getters.time.end.replace(/-/g, '.')
      }
      return timer
    }
  }
}
</script>

<style lang="scss" scoped>
.one-card-one {
  // min-width: 186px;
  background-image: url(data:image/png;base64,),
    url(http://likede2-admin.itheima.net/img/task.66b715b7.png);
  background-repeat: no-repeat;
  background-position: left top, right bottom;
  border: 0px;
  height: 126px;
  border-radius: 15px;
  padding: 20px;
  background-color: rgb(235, 243, 254);
  position: relative;
  .top-text-1 {
    .top-text-1-1 {
      position: absolute;
      top: 20px;
      left: 30px;
      font-weight: 500;
    }
    .top-text-1-2 {
      position: absolute;
      top: 23px;
      left: 100px;
      color: rgb(158, 159, 160);
      font-size: 12px;
      font-weight: 400;
    }
  }
  .buttom-text-1 {
    .buttom-text-1-1 {
      font-size: 40px;
      display: grid;
      grid-template-columns: 25% 25% 25% 25%;
      margin-top: 10px;
      color: rgb(10, 35, 111);
      text-shadow: 1px 1px 10px rgba(10, 35, 111, 0.5);
      .buttom-text-1-1-1 {
        left: 40px;
        top: 50px;
        font-weight: 600;
      }
      .buttom-text-1-1-2 {
        left: 120px;
        top: 50px;
        font-weight: 600;
      }
      .buttom-text-1-1-3 {
        left: 40px;
        top: 50px;
        font-weight: 600;
      }
      .buttom-text-1-1-4 {
        left: 40px;
        top: 50px;
        font-weight: 600;
      }
    }
    .buttom-text-1-2 {
      font-size: 12px;
      display: grid;
      grid-template-columns: 25% 25% 25% 25%;
      color: rgb(156, 174, 219);
    }
  }
}
</style>
