<template>
  <div>
    ...
    <div class="main">
      <!-- 给子组件传数据 -->
      <!-- v-model等于 :value = 'item.item.goods_count' @input="item.goods_count = $event"  -->
      <MyGoods
        v-model="item.goods_count"
        v-for="item in list"
        :key="item.id"
        :item="item"
        @changeFn="item.goods_state = !item.goods_state"
      ></MyGoods>
    </div>
    <MyFooter
      :state="isAll"
      @changeFnn="isAll = !isAll"
      :priceSum="priceSum"
      :countSum="countSum"
    ></MyFooter>
  </div>
  ...
  computed: {
    ...
    priceSum() {
      return this.list
        ...
        .reduce((sum, item) => item.goods_price * item.goods_count + sum, 0);
    },
    countSum() {
      return this.list
        .filter((item) => item.goods_state)
