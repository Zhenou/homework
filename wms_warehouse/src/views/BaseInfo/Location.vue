        ...
        <el-form ref="form" :model="form">
          <el-card class="box-card">
            <el-row :gutter="20">
              ...
              <el-col :span="6">
                <div class="button">
                  <el-button type="primary" round @click="getLocationList('search')"
                    >搜索</el-button
                  >
          ...
          >
            <template slot-scope="scope">
              ...
              <el-button
                type="text"
                size="small"
                @click="changeLocationState(scope.row)"
...
import { getLocationList, DeleteLocation, changeLocationState } from '@/api/Location'
export default {
  data () {
    return {
  ...
  methods: {
    ...
    async getLocationList (tag) {
      this.tableLoading = true
      let res = {}
      if (tag === 'search') {
        // 搜索请求
        const data = this.form
        res = await getLocationList(data)
      } else if (tag === 'default') {
        // 默认请求
        res = await getLocationList({ size: 10 })
      } else if (!tag) {
        // 页码请求
        res = await getLocationList({
    ...
    changeLocationState (row) { // 改变状态
      this.$confirm(
        ...
        .then(async () => {
          await changeLocationState({ status: row.status ? 0 : 1, id: row.id })
    ...
    async addTopList () { // 编辑
      this.$router.push({ name: 'LocationNewEdit', params: { id: 'null' } })
    },
    downLoad () { // 下载模版
